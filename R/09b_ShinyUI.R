shiny_ui <- fluidPage(
  title = 'tviblindi',
  ## CSS tags
  # tviblindi title font
  tags$style(HTML('#app_title { font-size: 32px; font-family: monospace }')),
  # dendrogram percentage cutoff slider style
  tags$style(type = 'text/css', '.irs-slider { width: 15px; height: 15px; top: 22px }'),
  # window size
  tags$head(tags$script(HTML('$(window).resize(function(event){ var w = $(this).width(); var h = $(this).height(); var obj = {width: w, height: h}; Shiny.onInputChange("windowSizeChange", obj); }); '))),
  # size of gating layout modal window
  tags$head(tags$style('#modal_layout_gating .modal-lg { width: 1850px }')),
  tags$head(tags$style('#modal_layout_gating_termini .modal-lg { width: 1850px }')),
  # position of progress bar pop-up (for triangulation of walks)
  tags$head(tags$style('.shiny-notification { position: fixed; top: 20%; left: 10%; font-size: 24px')),
  # top bar button colours
  tags$head(tags$style(HTML('#btn_left_show_gating {background-color:#337ab7; color:white}')), tags$style(HTML('#btn_help {background-color:#337ab7; color:white}')),),
      ...
      tabsetPanel(
        tabPanel(
          ...
          column(
            ...
            actionButton('btn_dendrogram_update_walks_selection', '', icon = icon('glyphicon glyphicon-transfer', lib = 'glyphicon')) ##METHOD CHANGED
          ),
          column(
            width = 2,
            style = 'text-align:  right;
