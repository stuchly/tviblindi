...
#' Convert sparse adjacency matrix to similarity matrix - symmetrized case
#'
#' \code{knn.adj2spadjsim1} converts a sparse adjacency matrix, as generated by \code{knn.raw2adj}, to a similarity matrix using some kernel function.
#'  Distinctly from an adjacency matrix, entries in a similarity matrix are dependent upon, but not equivalent to, distances.
#'  They are calculated using a probability density function (PDF). Standard deviation of distribution is taken as median distance or some function thereof.
#'  The particular distance value is supplied as the quantile parameter for the PDF.
#'
...
knn.adj2spadjsim1<-function(adj,kernel=c("Exp"),epsilon=NULL){
    ...
    if (kernel=="ExpMr") {
        ...
        mmm<-lapply(1:N,FUN=function(i,adj_s,i_list) max(adj_s[i_list[[i]],3]),i_list=i_list,adj_s=adj_s)
        adj[3,]<-unlist(lapply(1:N,FUN=function(i,i_list,mmm) adj_s[i_list[[i]],3]/mmm[[i]],i_list=i_list,mmm=mmm))

        adj[3,]<-exp(-adj[3,])
        indices<-unlist(i_list)
        adj[1,]<-adj_s[indices,1]
        adj[2,]<-adj_s[indices,2]
    }
    ...
    if (kernel=="ExpMer2") {
        adj_s<-summary(Matrix::sparseMatrix(i=adj[1,],j=adj[2,],x=adj[3,],dims=c(N,N)))
        i_list<-split(1:nrow(adj_s),as.integer(adj_s[,1]))
        mmm<-lapply(1:N,FUN=function(i,adj_s,i_list) 2*median(adj_s[i_list[[i]],3]),i_list=i_list,adj_s=adj_s)
        adj[3,]<-unlist(lapply(1:N,FUN=function(i,i_list,mmm) adj_s[i_list[[i]],3]/mmm[[i]],i_list=i_list,mmm=mmm))
     ...
     if (kernel=="SEMr") {
        ...
        i_list<-split(1:nrow(adj_s),as.integer(adj_s[,1]))
        mmm<-lapply(1:N,FUN=function(i,adj_s,i_list) max(adj_s[i_list[[i]],3]),i_list=i_list,adj_s=adj_s)
        adj[3,]<-unlist(lapply(1:N,FUN=function(i,i_list,mmm) (adj_s[i_list[[i]],3])^2/mmm[[i]],i_list=i_list,mmm=mmm))
      ...
      if (kernel=="SEMer2") {
        ...
        i_list<-split(1:nrow(adj_s),as.integer(adj_s[,1]))
        mmm<-lapply(1:N,FUN=function(i,adj_s,i_list) median(adj_s[i_list[[i]],3]),i_list=i_list,adj_s=adj_s)
        adj[3,]<-unlist(lapply(1:N,FUN=function(i,i_list,mmm) adj_s[i_list[[i]],3]/(sqrt(2)*mmm[[i]]),i_list=i_list,mmm=mmm))
