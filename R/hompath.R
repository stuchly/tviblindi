...
#' @param X matrix of vertex coordinates together with indices mapping), with rows corresponding to vertices. Result of \code{select_unique_points}
...
#'
#' @export
triangulate_pathways<-function(walks,X,cmplx){
    sel<-1:length(walks$starts)
    ## ss<-which(unlist(lapply(cmplx,FUN=function(x) return(length(x)<=3)))) #if original cmplx!!
    ## cmplx<-cmplx[ss]
    ##Xmap<-X$map
    ##X<-X$X
    ...
    for (i in sel){
        j<-j+1
        out[[j]]<-ss[onepath_triangulation(select_paths_points(walks,i),X,hc,g)]
    }
    clear(hc)
    rm(hc)
    return(out)
