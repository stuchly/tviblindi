shiny_server <- function(
  ...
  fcs_subset_idcs <- tv$fcs_subset_idcs
  markers <- colnames(tv$data)
  
  if ((is.null(fcs_subset_idcs) && !is.null(input_ff) && nrow(input_ff) != nrow(tv$data)) || (!is.null(fcs_subset_idcs) && length(fcs_subset_idcs) != nrow(tv$data))) {
    stop('Number of events in expression matrix incongruent with dimensionality of input FCS file. Did you misuse the fcs_subset_idcs parameter?')
  }
  ...
  observeEvent(input$btn_help, {
    showModal(modalDialog(
      ...
      HTML("This interface is a tool to allow for discrimination within a set of canonical developmental trajectories in input data, as well as between canonical and aberrant trajectories. Some trajectories will be biologically relevant, others not.
             ...
             (Additionally, the choice of homology classes can be refined using the HCP dendrogram: once a subtree is selected, the homology classes used to generate splits in that subtree can be fed back as inputs to the persistence diagram by pressing the <b>TRANSFER</b> button showing arrows going in opposite directions.)
             <br><br>
  ...
  output$plot_termini <- renderPlot({
    if (react$image_export.termini) {
      ...
      par(mar = c(1, 1, 1, 1))
      layout.raster <- scattermore::scattermore(
      ...
      plot(layout.raster)
      layout.raster.origin <- scattermore::scattermore(
      ...
      layout.raster.termini <- scattermore::scattermore(
      ...
      if (tv$show_all_fates) {
        layout.raster.fates <- scattermore::scattermore(
      ...
      graphics::rasterImage(as.raster(matrix(colorRampPalette(c('yellow', 'brown', 'red'))(20), ncol = 1)), xleft = 465, ybottom = 0, xright = 480, ytop = 485)
      ...
      if (!is.null(tv$show_all_fates) && tv$show_all_fates) {
        points(
      ...
      graphics::rasterImage(as.raster(matrix(colorRampPalette(c('yellow', 'brown', 'red'))(20), ncol = 1)), 1.07, 0, 1.1, 0.9)
  ...
  output$plot_persistence <- renderPlot({
    if (!is.null(react$persistence)) {
      quantiles <- stats::quantile(react$persistence_diagram$yplot)
      sizes <- max(1, sapply(react$persistence_diagram$yplot, function(val) dplyr::first(which(quantiles >= val)) - 1))
  ...
  observeEvent(input$switch_persistence_death_on_x_axis, {
    ...
    react$persistence_diagram <- .persistence_diagram(
      react$persistence,
      react$persistence.death_birth_ratio, 
      react$persistence.death_on_x_axis
    )
  })
    ...
    if (react$dendrogram_ready) {
      ...
      isolate({
        if ((length(idcs_selected) > 0 && !is.null(react$representations)) || react$dendrogram_redraw_zoom) {
          ...
          simplices_selected            <- react$persistence$idcs$death[idcs_selected]
  ...
  observeEvent(react$trajectories_to_pin, {
    if (is.null(react$output_ff)) {
      if (is.null(fcs_subset_idcs)) {
        ...
        layout_X[fcs_subset_idcs] <- layout.df[[react$layout_name]]$X * 100
        layout_Y[fcs_subset_idcs] <- layout.df[[react$layout_name]]$Y * 100
        ps           <- rep(-100, nrow(tv$data))
        ps[fcs_subset_idcs] <- as.numeric(as.factor(tv$pseudotime[[react$pathmodel_name]]$res))
        labels_out<-matrix(-100,ncol=length(labels),nrow=nrow(tv$data))
        colnames(labels_out)<-paste("label",names(labels),sep="_")
        for (i in 1:length(labels)) labels_out[fcs_subset_idcs,i]<-labels[[i]]
      }
  ...
  observeEvent(input$btn_pin_batch_name, {
    ...
    if (input$input_pin_batch_name != '') {
      ...
      react$output_ff <- add_pathways_info_to_FCS(ff                       = react$output_ff,
                                                  pseudotime               = tv$pseudotime[[react$pathmodel_name]],
                                                  all_walks                = walks,
                                                  trajectories_of_interest = react$trajectories_to_pin,
                                                  id                       = input$input_pin_batch_name,
                                                  fcs_subset_idcs               = fcs_subset_idcs)
      removeModal()
    } else {
  ...
  observeEvent(input$btn_trajectories_clear_pinned_trajectories, {
    if (is.null(react$output_ff)) {
      if (is.null(fcs_subset_idcs)) {
        ...
        layout_Y[fcs_subset_idcs] <- layout.df[[react$layout_name]]$Y * 100
        ps           <- rep(-100, nrow(tv$data))
        ps[fcs_subset_idcs] <- tv$pseudotime[[react$pathmodel_name]]$res
        labels_out<-matrix(-100,ncol=length(labels),nrow=nrow(tv$data))
        colnames(labels_out)<-paste("label",names(labels),sep="_")
        for (i in 1:length(labels)) labels_out[fcs_subset_idcs,i]<-labels[[i]]
      }
