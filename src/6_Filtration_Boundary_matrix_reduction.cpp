  ...
  const int           maxdimension)
{
  ...
  for (; iCmplx != cmplx.end(); ++iCmplx) {
    ...
    if (cmplxVec.size() > 1)
    {
      boundary_matrix.set_col(col, phat::column(cmplxVec.begin(), cmplxVec.end()));
    } else {
  ...
  for(phat::index col_idx = 0; col_idx < boundary_matrix.get_num_cols(); ++col_idx)
  {
    if(!boundary_matrix.is_empty(col_idx))
    {
      ...
      nonzero_col.push_back(j + 1);  // re-index to 1-based
      dim.push_back(boundary_matrix.get_dim(col_idx) - 1); // corresponds to dim-1 cycle
      low.push_back(boundary_matrix.get_max_index(col_idx) + 1);  // re-index to 1-based
      int i = 0;
      for(phat::index idx = 0; idx < (phat::index)temp_col.size(); ++idx)
      {
        v_loc[i]=temp_col[idx]+1;i++; // re-index to 1-based
      }
      v.push_back(v_loc);          
    }
    ++j;     
  ...
  return Rcpp::List::create(
    Rcpp::Named("boundary",    Rcpp::wrap(v)),
    Rcpp::Named("nonzero_col", Rcpp::wrap(nonzero_col)),
    Rcpp::Named("low",         Rcpp::wrap(low)),
    Rcpp::Named("dim",         Rcpp::wrap(dim))
  );
}
