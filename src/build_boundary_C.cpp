...
std::vector< std::vector<int> > subsetsB (   // returns all relevant subsets of vector, gained by removing one vertex
...
// [[Rcpp::export]]
RcppExport SEXP build_boundary_C (   // provided a list of simplices, makes list of previous simplices which are boundaries
  ...
  for (int ii = 0; ii < filtration.size(); ++ii) {
    std::vector< std::vector<int> > ss = subsetsB (filtration[ii]);
    std::vector<int> h1;
